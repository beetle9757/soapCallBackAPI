package com.infrasofttech.api.callback;
...
import com.infrasofttech.config.fts.callback.stub.AMLScan;
import com.infrasofttech.config.fts.callback.stub.AMLScanResponse;
import com.infrasofttech.config.fts.callback.stub.AmlScanRequest;
import com.infrasofttech.config.fts.callback.stub.NBFAMLHEADER;
import com.infrasofttech.config.fts.callback.stub.ObjectFactory;
import com.infrasofttech.config.fts.callback.stub.Response;
...
public class WssSecurityRequestTest extends WebServiceGatewaySupport {
	...
	public void doCallback() throws Exception {
		...
		String callbackMessage = """
				...
				""";
		String callBackAPIUrl = properties.getProperty("FTS_CALL_BACK_URL");
		try {
	...
	private boolean populateWebServiceTemplate() {
		...
		try {
	        ...
	        userNameInterceptor.setSecurementUsername(properties.getProperty("FTS_CALL_BACK_USER_NAME"));
	        userNameInterceptor.setSecurementPassword(properties.getProperty("FTS_CALL_BACK_PASSWORD"));
			...
			SSLContext sslContext = new SSLContextBuilder().loadTrustMaterial(null, acceptingTrustStrategy).loadKeyMaterial(keyStore, properties.getProperty("FTS_CALL_BACK_CERTIFICATE_PASSWORD").toCharArray()).build();
			
			List<Header> headers = new ArrayList<>();
	        BasicHeader authHeader = new BasicHeader("NBF-APIKey", properties.getProperty("FTS_CALL_BACK_API_KEY"));
			...
			UsernamePasswordCredentials credentials = new UsernamePasswordCredentials(properties.getProperty("FTS_CALL_BACK_USER_NAME"), properties.getProperty("FTS_CALL_BACK_PASSWORD"));
			...
			SoapActionInterceptor actionInterceptor = new SoapActionInterceptor(properties.getProperty("FTS_CALL_BACK_SOAP_ACTION"));
			...
			webServiceTemplate.setDefaultUri(properties.getProperty("FTS_CALL_BACK_URL"));
