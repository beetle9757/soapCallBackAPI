package com.infrasofttech.api.callback.uaefts;
...
import com.infrasofttech.config.uaefts.fts.callback.stub.FTSAMLUpdateAMLInput;
import com.infrasofttech.config.uaefts.fts.callback.stub.ObjAMLReq;
import com.infrasofttech.config.uaefts.fts.callback.stub.ObjectFactory;
import com.infrasofttech.config.uaefts.fts.callback.stub.UpdateAMLRequest;
import com.infrasofttech.config.uaefts.fts.callback.stub.UpdateAMLRequestResponse;
...
public class TestUaeftsFtsCallBackApi extends WebServiceGatewaySupport {
	...
	public void doCallback() throws Exception {
		...
		String callBackAPIUrl = properties.getProperty("UAEFTSFTS_CALL_BACK_URL");
		try {
			...
			FTSAMLUpdateAMLInput finalInputRequest = objFactory.createFTSAMLUpdateAMLInput();
			finalInputRequest.setUpdateAMLRequest(request);

//			com.infrasofttech.config.uaefts.fts.callback.stub.FTSAMLUpdateAMLOutput response = (com.infrasofttech.config.uaefts.fts.callback.stub.FTSAMLUpdateAMLOutput) webServiceTemplate
...
//			marshaller.setPackagesToScan("com.infrasofttech.config.uaefts.fts.callback.stub");
			marshaller.setContextPath("com.infrasofttech.config.uaefts.fts.callback.stub");
			...
			userNameInterceptor.setSecurementUsername(properties.getProperty("UAEFTSFTS_CALL_BACK_USER_NAME"));
			userNameInterceptor.setSecurementPassword(properties.getProperty("UAEFTSFTS_CALL_BACK_PASSWORD"));

			KeyStore keyStore = KeyStore.getInstance("PKCS12");
			keyStore.load(new FileInputStream(properties.getProperty("UAEFTSFTS_CALL_BACK_CERTIFICATE_PATH")),
					properties.getProperty("UAEFTSFTS_CALL_BACK_CERTIFICATE_PASSWORD").toCharArray());
			...
			BasicHeader authHeader = new BasicHeader("NBF-APIKey",
					properties.getProperty("UAEFTSFTS_CALL_BACK_API_KEY"));
			...
			HttpComponentsMessageSender messageSender = new HttpComponentsMessageSender() {
				...
				protected HttpContext createContext(URI uri) {
					...
					credsProvider.setCredentials(authScope,
							new UsernamePasswordCredentials(properties.getProperty("UAEFTSFTS_CALL_BACK_USER_NAME"),
									properties.getProperty("UAEFTSFTS_CALL_BACK_PASSWORD")));
...
//			messageSender.setCredentials(credentials);
			...
			SoapActionUaeftsInterceptor actionInterceptor = new SoapActionUaeftsInterceptor(
					properties.getProperty("UAEFTSFTS_CALL_BACK_SOAP_ACTION"));
			...
			webServiceTemplate.setDefaultUri(properties.getProperty("UAEFTSFTS_CALL_BACK_URL"));
	...
	private Properties loadCallbackProperties() throws Exception {
		...
		while (rootDirectories.hasNext()) {
			...
			File propertiesFile = new File(absolutePath + File.separator + "kiya-ai-aml" + File.separator
					+ "UNIVIUS-AML-CONFIG" + File.separator + "SERVICE" + File.separator + "ServiceConfig.properties");
